...
fn main()  {
    ...
    let content = format!("{}",arr.clone().into_iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
    let _ = fs::write("./assets/input_sorting.txt", content);

    let n = arr.len();
    let sorted_arr = bubble_sort(n, arr);
    
    let content = format!("{}",sorted_arr.clone().into_iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
